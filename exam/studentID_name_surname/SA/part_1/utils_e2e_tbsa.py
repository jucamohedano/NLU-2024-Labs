...
def build_dataset(ds_name, input_win=1, tagging_schema='BIO', stm_win=1):
    ...
    # loaded datasets
    train_set, train_set_label_mapping = read_data(path=train_path)
    test_set, test_set_label_mapping = read_data(path=test_path)
    ...
    for i in range(n_train):
        record = train_set[i]
        record_with_label = train_set_label_mapping[i]
        if i in val_sample_ids:
            val_set.append(record)
            val_set_label_mapping.append(record_with_label)
        else:
            tmp_train_set.append(record)
            tmp_train_set_label_mapping.append(record_with_label)
    train_set = [r for r in tmp_train_set]
    train_set_label_mapping = [r for r in tmp_train_set_label_mapping]

    return train_set, train_set_label_mapping, val_set, val_set_label_mapping, \
            test_set, test_set_label_mapping, vocab, char_vocab, ote_tag_vocab, ts_tag_vocab
